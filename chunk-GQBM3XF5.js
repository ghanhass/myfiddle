...
function findCompositionRange(view, changes, headPos) {
  ...
  for (let parent = textNode.parentNode; ; parent = parent.parentNode) {
    ...
    else if (parent != view.contentDOM) marks2.push({
      node: parent,
...
function placeholder(content2) {
  let plugin = ViewPlugin.fromClass(class {
    constructor(view) {
      this.view = view;
      this.placeholder = content2 ? Decoration.set([Decoration.widget({
  ...
  return typeof content2 == "string" ? [plugin, EditorView.contentAttributes.of({
...
var HoverPlugin = class {
  ...
  mousemove(event) {
    ...
    if (active.length && tooltip && !isInTooltip(tooltip.dom, event) || this.pending) {
      let {
        pos
      } = active[0] || this.pending, end = (_b = (_a2 = active[0]) === null || _a2 === void 0 ? void 0 : _a2.end) !== null && _b !== void 0 ? _b : pos;
...
var NodeSet = class _NodeSet {
  ...
  */
  constructor(types2) {
    this.types = types2;
    for (let i = 0; i < types2.length; i++) if (types2[i].id != i) throw new RangeError("Node type ids should correspond to array positions when creating a node set");
  }
  /**
...
function syntaxIndentation(cx, ast, pos) {
  ...
  if (inner != stack.node) {
    let add2 = [];
    for (let cur = inner; cur && !(cur.from < stack.node.from || cur.to > stack.node.to || cur.from == stack.node.from && cur.type == stack.node.type); cur = cur.parent) add2.push(cur);
    for (let i = add2.length - 1; i >= 0; i--) stack = {
      node: add2[i],
      next: stack
    };
  }
...
function indentOnInput() {
  return EditorState.transactionFilter.of((tr) => {
    ...
    let doc2 = tr.newDoc, {
      head
    } = tr.newSelection.main, line = doc2.lineAt(head);
...
//# sourceMappingURL=chunk-GQBM3XF5.js.map
